...
if ($_GET['op'] == 'unbind') {
	...
	echo "<tr class='header'><th>openid</th><th>uid</th><th>username</th><th>from</th>".$thadd."</tr>";
	foreach ($fquery as $fresult){
		$member = C::t('#hux_wx#hux_common_member')->fetch_by_uid($fresult['uid']);
		$fresult['username'] = $member['username'];
		$fresult['from'] = strlen($fresult['openid']) == 32 ? 'yx' : 'wx';
		if ($appnum) {
			include DISCUZ_ROOT.'./source/plugin/hux_wx/mod/unbind/lang/lang.'.currentlang().'.php';
		...
		echo "<tr><td>".$fresult['openid']."</td><td>".$fresult['uid']."</td><td><a href='home.php?mod=space&uid=".$fresult['uid']."' target='_blank'>".$fresult['username']."</a></td><td>".$fresult['from']."</td>".$fresult['op']."</tr>";
	}
	showtablefooter();
