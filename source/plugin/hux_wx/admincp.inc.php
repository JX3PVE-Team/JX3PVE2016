...
if($_GET['op'] == 'add') {
	if ($_GET['formhash'] == formhash()) {
		...
		$apparr = array(
			...
			'configs' => serialize($appnamebak['configs']),
			'paixu' => 0,
	...
	if ($_GET['formhash'] == formhash()) {
		...
		if ($appver['appver'] == $appnamebak['appver']) {
			...
			C::t('#hux_wx#hux_wx_config')->update_appver_configs_by_appid($_GET['appid'],$appnamebak['appver'],serialize($appnamebak['configs']));
			require_once libfile('function/plugin');
	...
	if (submitcheck('submit')) {
		if (($appnum['appid'] == 'at' && $_GET['appcmd'] != '@') || ($appnum['appid'] == 'login' && $_GET['appcmd'] != '#') || ($appnum['appid'] == 'robot' && $_GET['appcmd'] != 'robot')) {
			cpmsg('robot|#|'.$Plang['aterr'], 'action=plugins&operation=config&do='.$pluginid.'&identifier=hux_wx&pmod=admincp', 'succeed');
		} else {
			foreach ($_GET as $k => $v) {
				if (!in_array($k,array('action','operation','do','identifier','pmod','op','formhash','scrolltop','anchor','submit','appid','appcmd','appcmdtxt','paixu'))) {
					$configs_data[$k] = $v;
				}
			}
			$data_arr = array(
		...
		if ($appnum) {
			$appconfig = unserialize($appnum['configs']);
		}
